...
module.exports = function getLockfile(packageFile, date, {
	...
	return Promise.all([tmpDirP, copyPkg]).then(([tmpDir]) => new Promise((resolve, reject) => {
		...
		exec(
			`npm install --ignore-scripts --package-lock --package-lock-only${date ? ` --before=${date}` : ''}${only ? ` --only=${only}` : ''}`, /*Vulnerability name: Indirect uncontrolled command line	Vulnerability description: Forwarding command-line arguments to a child process executed within a shell may indirectly introduce command-line injection vulnerabilities.	Vulnerability message: This command depends on an unsanitized [["command-line argument"|"relative:///bin.js:17:2:21:2"]].*/
			{
				cwd: tmpDir,
				env: {
