...
module.exports = function getLockfile(packageFile, date, {
	...
	return Promise.all([tmpDirP, copyPkg]).then(([tmpDir]) => new Promise((resolve, reject) => {
        ...
        // Sanitize inputs
        const sanitizedDate = date ? ` --before=${sanitizeInput(date)}` : '';
        const sanitizedOnly = only ? ` --only=${sanitizeInput(only)}` : '';

		exec(
			`npm install --ignore-scripts --package-lock --package-lock-only${sanitizedDate}${sanitizedOnly}`,
			{
				cwd: tmpDir,
				env: {
