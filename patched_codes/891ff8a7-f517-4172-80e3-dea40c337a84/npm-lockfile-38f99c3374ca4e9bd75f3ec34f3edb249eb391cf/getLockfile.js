...
module.exports = function getLockfile(packageFile, date, {
    ...
    return Promise.all([tmpDirP, copyPkg]).then(([tmpDir]) => new Promise((resolve, reject) => {
        ...
        // Use execFile which takes arguments as an array rather than a single string
        execFile('npm', args, { cwd: tmpDir, env: { PATH } }, (error, stdout, stderr) => {
            if (error) {
                reject(error);
                return;
            }
